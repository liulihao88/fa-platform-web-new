<template>
  <div class="p-2">
                      ...
                      <template #label>
                        <div style="display: flex; justify-content: space-between; width: 100%">
                          ...
                          <span v-if="!isRelease" @click="handleAddFlowClick" class="knowledge-txt">
                            <Icon icon="ant-design:plus-outlined" size="13" style="margin-right: 2px"></Icon>
                            添加
                          </span>
                        </div>
                      </template>
                ...
                <a-col :span="24" class="mt-10">
                  <div class="prologue-chunk-edit">
                    <a-form-item
                      :labelCol="labelCol"
                      :wrapperCol="wrapperCol"
                      v-bind="validateInfos.presetQuestion"
                      style="margin-bottom: 0"
                    >
                      <template #label>
                        <div class="prompt-title-padding item-title space-between">
                      ...
                      <div style="padding: 0 10px" v-if="presetQuestionList.length > 0">
                        <draggable
                          ...
                          item-key="key"
                          v-model="presetQuestionList"
                          @end="presetQuestionEnd"
                        >
                      ...
                      <template #label>
                        <div style="display: flex; justify-content: space-between; width: 100%; margin-right: 2px">
                          ...
                          <div v-if="!isRelease" @click="handleParamSettingClick('model')" class="knowledge-txt">
                            <Icon icon="ant-design:setting-outlined" size="13" style="margin-right: 2px"></Icon>
                            参数配置
                          </div>
                        </div>
                      </template>
                    ...
                    >
                      ...
                      <a-row :span="24">
                        <a-col
                          ...
                          v-if="knowledgeDataList && knowledgeDataList.length > 0"
                        >
                            ...
                            <div style="display: flex; width: 100%; justify-content: space-between">
                              <div>
                                ...
                                <span
                                  ...
                                  style="color: #e03e2d; text-decoration: line-through"
                                  v-if="item.type"
                                >
                                  {{ item.name }}
                                </span>
                              ...
                              <Icon
                                v-if="!isRelease"
                                @click="handleDeleteKnowledge(item.id)"
                                icon="ant-design:close-outlined"
                                size="20"
                                class="knowledge-icon"
                              ></Icon>
    ...
    <AiAppParamsSettingModal
      @register="registerParamsSettingModal"
      @ok="handleParamsSettingOk"
    ></AiAppParamsSettingModal>
    <!--  Ai应用新增编辑弹窗  -->
    <AiAppModal @register="registerAiAppModal" @success="handelEditSuccess"></AiAppModal>
    <!-- Ai生成器   -->
    <AiAppGeneratedPromptModal
      @register="registerAiAppPromptModal"
      @ok="handleAiAppPromptOk"
    ></AiAppGeneratedPromptModal>
    <!--  Ai快捷指令  -->
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const { resetFields, validate, validateInfos, validateField } = useForm(formState, validatorRules, {
      immediate: false,
    })
