...
import { data1 } from '@/views/test/mock/data'

const sourceColumns = data1.dataBlockStucts ?? []
const noMappingFields = data1.noMappingTitle
  ?.split(',')
  .map((item) => item.trim())
  .filter(Boolean)

const tableData = computed(() => {
  const maxRowCount = Math.max(...sourceColumns.map((item) => item.datas?.length ?? 0), 0)
