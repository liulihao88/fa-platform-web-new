﻿<template>
      ...
      <a-form ref="formRef" class="search-form" name="advanced_search" :model="formState" @finish="onSearch">
        ...
        <a-row gutter="16">
          <a-col :span="24">
            <a-row :gutter="24">
              <a-col :span="6">
                <a-form-item name="folder" label="文件夹" :labelCol="{ span: 4 }" :wrapperCol="{ span: 18 }">
                  <a-input v-model:value="formState.folder" placeholder="请输入文件夹名称" />
                </a-form-item>
              </a-col>
              <a-col :span="6">
                <a-form-item name="fileName" label="文件名称" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18 }">
        ...
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-else-if="column.key === 'operation'">
            <div class="table-operations">
              <a-button v-if="checkFilesNames(record)" class="ml1" size="small" type="primary" @click="handleTitleConfigClick(record)">
                标题配置
              </a-button>
              <a-button v-if="checkFilesNames(record)" class="ml1" size="small" type="primary" @click="handleEditFileClick(record)">
                转换查看
              </a-button>
    ...
    <div>
      <a-card style="height: 100%">
        <div class="panel-controls">
          ...
          <a-button type="primary" size="small" @click="toggleRightPanel" :icon="rightPanelVisible ? '->' : '<-'" class="ml2">
  ...
  <!-- 上传文件的Modal弹框 -->
  <BasicModal v-model:visible="uploadModalVisible" title="上传文件" width="800px" :maskClosable="false" :keyboard="false" :footer="null">
    <div>
      ...
      <a-card>
        ...
        >
          <div class="upload-dragger-content">
            ...
            <p class="ant-upload-hint">支持扩展名 .xls .xlsx .csv .pdf .zip</p>
          </div>
        </a-upload-dragger>
      ...
      <a-row :gutter="16">
        ...
        <a-col span="6">
          <a-card title="文件列表" size="small">
            <div class="pagination-controls">
              ...
              <a-button :disabled="filePagination.current === filePagination.totalPage" @click="nextPage" size="small">下一页</a-button>
            </div>
        ...
        <a-col span="16">
          <a-card title="转换信息" size="small">
            <a-form ref="convertFormRef" :model="convertFormState" :label-col="{ span: 8 }" :wrapper-col="{ span: 14 }">
              ...
              <a-form-item labelWrap="true" label="上述银行是否正确，需要重新指定">
                ...
                <JSearchSelect
                  ...
                  v-model:value="convertFormState.dataOrg"
                  placeholder="请选择"
                  allow-clear
                ></JSearchSelect>
              </a-form-item>
          ...
          <a-card title="文件信息" size="small">
            <a-row :gutter="16">
              <a-col :span="8">
                <div>
                  <strong>文件名称：</strong>
                  {{ currentTitleConfigFile?.folder || '-' }}{{ currentTitleConfigFile?.sourceFile || currentTitleConfigFile?.fileName || '-' }}
                </div>
              </a-col>
              <!--              <a-col :span="8">-->
                    ...
                    <template v-else-if="column.dataIndex && column.dataIndex.startsWith('col')">
                      <div v-if="record.type === 'newMetaData'" class="config-select-cell">
                        <JSearchSelect
                          ...
                          style="width: 100%"
                          :disabled="
                            isCurrentSheetConfigured ||
                            (dataBlock.dataBlockStucts[parseInt(column.dataIndex.replace('col', ''))].faFileParameter.oriMetaData ? true : false)
                          "
                        />
                      </div>
                      <div v-else-if="record.type === 'titleColName'">
                        <span
                          :style="{
                            color: dataBlock.dataBlockStucts[parseInt(column.dataIndex.replace('col', ''))].faFileParameter.oriMetaData
                              ? 'inherit'
                              : 'red',
                          }"
                        >
...
已完成	102	合并完成
  ...
  const isCurrentSheetConfigured = computed(() => {
    ...
    const currentSheet = currentTitleConfigFile.value.filePages.find((sheet) => sheet.pageId === activeTitleConfigSheet.value);
  ...
  const checkFilesNames = (record) => {
    ...
    return !(lowerFile1.endsWith('.zip') || lowerFile1.endsWith('.rar') || lowerFile2.endsWith('.zip') || lowerFile2.endsWith('.rar'));
  };
  ...
  const saveTitleConfig = async () => {
    try {
      ...
      // 准备请求参数
      const params = {
        faFileParameters: titleConfigData.value.result.flatMap((dataBlock) => dataBlock.dataBlockStucts.map((struct) => struct.faFileParameter)),
            ...
            return {
              ...
              dataBlockStucts: dataBlock.dataBlockStucts.map((struct) => {
                return {
                  ...
                  // 保存原始配置选项到结构中，用于过滤
                  availableOptions:
                    configResponse && Array.isArray(configResponse)
                      ? configResponse
                          .filter((option) => !dataBlock.dataBlockStucts.some((s) => s.faFileParameter.oriMetaData === option))
