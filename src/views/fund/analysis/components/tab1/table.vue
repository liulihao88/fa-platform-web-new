...
const useMapTextTitle = h('div', {}, [
  ...
  h('div', '2、有借贷方向时，借贷方向+交易金额，确定交易的金额'),
  h('div', '3、没有借贷方向时，当调整项为否时，对于金额字段，以正数表示交易金额为借方，调整项为是，则以负数表示为借方'),
  h('div', '4、正常情况下，正数表示贷方金额为贷方；正数表示结算金额为贷方，如果该Sheet的数据与此规则定义相反，请选择对应调整项为是'),
  h('div', '5、当未映射的字段不重要时，可以不做映射')
]);
