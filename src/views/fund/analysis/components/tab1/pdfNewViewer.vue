<template>
  <div class="pdf-viewer">
    <div class="pdf-controls">
      <button @click="zoomIn" class="control-btn" size="small">放大</button>
      <button @click="zoomOut" class="control-btn">缩小</button>
      <button @click="previousPage" :disabled="currentPage <= 1" class="control-btn">上一页</button>

      <button @click="nextPage" :disabled="!pdfDoc || currentPage >= pdfDoc.numPages" class="control-btn">
        下一页
      </button>
      <input
        type="number"
    ...
    <div ref="containerRef" class="pdf-container" @scroll="handleScroll">
      <div class="pages-wrapper" :style="{ height: totalHeight + 'px' }">
        ...
        >
          <!-- Canvas 用于渲染PDF -->
          <canvas :ref="(el) => setCanvasRef(el, pageData.pageNum)" class="pdf-canvas"></canvas>
        </div>
      </div>
