...
export function useGlobalLoading() {
  ...
  function loadingTrue(loadingText) {
    if (loadingText === true) {
      ...
      loadingNumberObj.value.true = loadingNumberObj.value.true
        ? loadingNumberObj.value.true + 1
        : 1;
    }
    if (loadingText === "loading1") {
      clearTimeout(timer.value);
      useLoading1.value = true;
      loadingNumberObj.value.loading1 = loadingNumberObj.value.loading1
        ? loadingNumberObj.value.loading1 + 1
        : 1;
    }
    if (loadingText === "loading2") {
      clearTimeout(timer.value);
      useLoading2.value = true;
      loadingNumberObj.value.loading2 = loadingNumberObj.value.loading2
        ? loadingNumberObj.value.loading2 + 1
        : 1;
    }
  }
