...
export default defineComponent({
  ...
  setup(props, { emit, refs }) {
    ...
    const handleAsyncFocus = () => {
      ...
      if (
        (isObject(selectedAsyncValue.value) || selectedAsyncValue.value?.length) &&
        isDictTable.value &&
        props.async
      ) {
      ...
      if (
        Array.isArray(selectedAsyncValue.value) &&
        selectedAsyncValue.value.length === 0 &&
        isDictTable.value &&
        props.async
      ) {
        if (pageNo > 2) {
      ...
      if (isDictTable.value) {
        ...
        if (!scrollLoading && isHasData && scrollTop + clientHeight >= scrollHeight - 10) {
          ...
          defHttp
            .get({
              url: `/sys/dict/loadDict/${props.dict}`,
              params: { pageSize: props.pageSize, keyword: keywordInfo, pageNo },
            })
            .then((res) => {
              loading.value = false
              if (res?.length > 0) {
